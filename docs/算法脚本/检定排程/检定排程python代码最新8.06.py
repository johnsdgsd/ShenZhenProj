...
# 1. 读取原始数据 
file_path = r"C:\Users\H5966\Desktop\检定排程\检定仓情况-20260805.xlsx"  
...
for month in months:
    ...
    while pending_batches:
        ...
        if future_need >= remain:
            ...
            est_minutes = int((est_date - base_date).total_seconds() / 60) if pd.notna(est_date) else 0
            schedule_batch(batch_no, dev_code, excess, is_priority=False, month=month, earliest_start=est_minutes)
            batch[2] = future_need
            new_pending.append(batch)
        else:
...
if pending_batches:
    ...
    while pending_batches:
        ...
        if not support:
            ...
            continue
        est_minutes = int((est_date - base_date).total_seconds() / 60) if pd.notna(est_date) else 0
        schedule_batch(batch_no, dev_code, remain, is_priority=False, month=last_month, earliest_start=est_minutes)
        pending_batches.popleft()
