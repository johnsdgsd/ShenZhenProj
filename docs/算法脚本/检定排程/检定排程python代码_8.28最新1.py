...
CHAMBER_TYPE_NAME_TO_CODE = {
    ...
    '普通/大变比低压CT兼容仓': 8,
    '普通/DBI低压CT兼容仓': 9,
    # 新版检定仓类型表别名（名称不同但含义对应同一VW_DEVICE_TYPE编码）
    '单相检定仓': 1,
    '单相/三相兼容仓': 2,
    '三相检定仓': 4,
    '终端/三相兼容仓': 5,
    '普通型低压电流互感器/大变比型低压电流互感器兼容仓': 8,
    '普通型低压电流互感器/DBI型低压电流互感器兼容仓': 9,
}
...
# 1. 读取原始数据
# ============================================================
file_path = r"C:\Users\H5966\Desktop\检定排程（新）\检定仓情况-20260825.xlsx"
...
for _, row in df_unqualified.iterrows():
    ...
    sampled = str(row['是否已抽检']).strip() == '是' if pd.notna(row['是否已抽检']) else True
    sample_qty = int(row['抽检数量']) if pd.notna(row['抽检数量']) else 0
...
def get_next_start_minutes(prev_end_minutes, duration, line_id, earliest_start_minutes=0):
    ...
    for _ in range(MAX_DAY_SEARCH):
        ...
        if start_abs < earliest_abs:
            # 直接从最早开始时间重新校验，由其落入的工作时段/次日逻辑保证不早于 earliest_start
...
schedule_details = []
batch_sample_done = {}  # 批次号 → 抽检完成时间(分钟)；值为0表示无需抽检
# 满仓补齐登记表：键 (批次号, 目标设备类型码) → {'idx': 记录下标, 'gap': 距满仓还差数量}
# 用于把同一“批次+目标类型”因按月/按目标比例分多段排程而残留的不满仓末批，逐段补齐到满仓
partial_registry = {}
...
def schedule_batch(batch_no, dev_code, quantity, is_priority, month, earliest_start=0, target_dev_code=None,
    ...
    # 避免同一批次被拆成多段（按月/按需求优先/按目标比例）后每一段都留下一个不满仓小批。
    ...
    while remaining > 0:
        ...
        sub_counter += 1
        # 首次检定的末批未装满时登记，供后续同批次+目标类型继续补齐（满仓优化）
...
def ensure_sample_inspection(batch_no, dev_code, month, earliest_start=0, target_splits=None, is_priority=False):
    ...
    is_priority: 该批次是否用于满足需求，决定抽检记录的“是否为需求优先”标记；
...
# 打出“是/否”，因此这里按量校正只针对首次检定（编码3），抽检记录不参与
...
print("是否为需求优先标记校正完成：按各设备码总需求扣除期初库存后的需检定数量，按完成时间先后标记")
...
def convert_dev_code_to_str(df):
    # 设备码/目标设备类型码/detectSchemeId 按文本输出，类别编码列保持数字编码
...
df_schedule_summary = df_details.groupby(
    ['月份', '检定线ID', '检定线名称', '检定设备类型编码', '检定设备类型名称', '设备码', '到货批次号', '是否为需求优先', '检定类型编码', '检定类型名称']
).agg(
    总检定数量=('每批数量', 'sum'),
    批次数=('内部批次号', 'nunique')
).reset_index()
...
df_first_insp = df_details[df_details['检定类型编码'] == DEFAULT_DETECT_TYPE_CODE]
for batch_no in df_first_insp['到货批次号'].unique():
    batch_data = df_first_insp[df_first_insp['到货批次号'] == batch_no].sort_values('预计开始时间')
...
print("\n" + "=" * 80)
print("=== 抽检逻辑验证 ===")
df_sample = df_details[df_details['检定类型编码'] == SAMPLE_DETECT_TYPE_CODE]
if len(df_sample) > 0:
    print(f"抽检记录共 {len(df_sample)} 条，涉及 {df_sample['到货批次号'].nunique()} 个批次，总计抽检 {df_sample['每批数量'].sum()} 台")
    for batch_no in df_sample['到货批次号'].unique():
        sample_end = df_sample[df_sample['到货批次号'] == batch_no]['预计完成时间'].max()
        first_subset = df_details[(df_details['到货批次号'] == batch_no) & (df_details['检定类型编码'] == DEFAULT_DETECT_TYPE_CODE)]
        first_start = first_subset['预计开始时间'].min() if len(first_subset) > 0 else None
        ok = first_start is not None and first_start >= sample_end
        print(f"  批次 {batch_no}: 抽检完成时间 {sample_end}, 首检最早开始 {first_start}, {'验证通过' if ok else '异常：首检早于抽检完成！'}")
else:
    print("无抽检记录（所有批次均已抽检或无抽检数量）")
...
print(f"VW_YES_NO_FLAG: 是(P)/否(N)（在'是否为需求优先'列中）")
